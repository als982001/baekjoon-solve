...
int Check(int curAlp, int curCop)
{
    if (curAlp >= maxReqAlp && curCop >= maxReqCop)
        return 0;
    
    if (check[curAlp][curCop] != INF)
        return check[curAlp][curCop];
    ...
    if (curAlp < maxReqAlp)
    {
        int result = Check(curAlp + 1, curCop) + 1;
    ...
    if (curCop < maxReqCop)
    {
        int result = Check(curAlp, curCop + 1) + 1;
        minCost = min(minCost, result);
    }
    ...
    {
        if (curAlp >= problem.reqAlp && curCop >= problem.reqCop)
        {
            int nxtAlp = min(curAlp + problem.rwdAlp, maxReqAlp);
            int nxtCop = min(curCop + problem.rwdCop, maxReqCop);
            
            if (nxtAlp == curAlp && nxtCop == curCop)
                continue;
            
            int result = Check(nxtAlp, nxtCop) + problem.cost;
            
            minCost = min(minCost, result);
        }
    }
    
    check[curAlp][curCop] = minCost;
...
int solution(int alp, int cop, vector<vector<int>> problems) {
    ...
    {
        allProblems.push_back({ problem[0], problem[1], problem[2], problem[3], problem[4] });
        
        if (maxReqAlp < problem[0])
            maxReqAlp = problem[0];
        
        if (maxReqCop < problem[1])
            maxReqCop = problem[1];
    }
    ...
    answer = Check(min(alp, maxReqAlp), min(cop, maxReqCop));
